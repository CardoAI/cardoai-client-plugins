---
title: Consumer — Point Of Sale Loans
tier: sub:consumer/point-of-sale-loans
kpi_count: 20
inherits_from:
  - consumer/common/kpis
---

# Consumer — Point Of Sale Loans
...
Shows how old the loans in the pool are, in months since origination. Useful because charge off risk typically peaks in the early months of a loan's life, so understanding pool seasoning helps anticipate when losses are most likely to emerge.
...
Splits the portfolio by the number of items included in the financed order to analyze exposure by order complexity. This helps identify whether larger, multi-item orders carry different credit risk profiles, repayment behavior, or return/refund patterns compared to single-item purchases.

**Computation**

```
Group by: Reporting Date, Items Count
...
Splits the portfolio by the product category of the financed item (e.g. Electronics, Apparel, Furniture, Health, Travel, Services) to analyze exposure concentration by merchandise type. This helps identify which product segments drive the highest loan volumes, assess category-level credit risk, and understand how repayment behavior and loss rates vary across different types of goods and services.

**Computation**

```
Group by: Reporting Date, Product Category
...
Splits the portfolio by the shipping fee charged on the financed order to analyze the relationship between delivery costs and loan performance. This helps identify whether orders with higher shipping amounts, often associated with bulkier or higher-value goods, exhibit different delinquency, return, or prepayment behavior compared to low or zero shipping cost orders.

**Computation**

```
Group by: Reporting Date, Shipping Amount
Metrics:
  • Total Loans = Count of Loan IDs
  • Open Loans = Count of Loan IDs where Loan is not Closed
  • UPB = Sum of UPB
